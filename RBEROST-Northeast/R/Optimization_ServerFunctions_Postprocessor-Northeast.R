...
read_all_streamcat_files <- function(scfiles) {
  ...
  if (is.null(scfiles) || nrow(scfiles) == 0) {
    return(NULL)
  }
  ...
  # If only one file, return a single dataframe with state abbreviation
  if (length(tmp) == 1) {
    single_df <- tmp[[1]]
    # Extract state abbreviation from filename (first 2 characters)
    state_abbrev <- substr(names(tmp)[1], 1, 2)
    single_df$StateAbbrev <- state_abbrev
    return(single_df)
  } else {
    # Multiple files - return list of dataframes
    return(tmp)
  }
}
...
get_loading_targets <- function(results_astable) {
  ...
  season_lims_lines <-
    results_astable[
      which(grepl(results_astable[, 1], pattern = "limits")),1
    ]
  ...
  loads_lims <- data.frame(
    ...
    Number = as.numeric(as.character(substr(targets, 2, nchar(targets)))),
    Limit =  as.numeric(
      as.character(str_extract(season_lims_lines, pattern = "\\d+(?:\\.\\d+)?(?=\\s*$)"))
    )
  ) %>%
    group_by(Nutrient, Number) %>% # Note, included to create annual estimates
    summarize(Limit = sum(Limit), .groups = "keep") %>%
    ungroup() %>%
    as.data.frame()
...
get_loading_bootstraps <- function(
  ...
  load_txt <- "inc_storage_load" # CTC edited: 1/8/2026
  ...
  if(
    ...
    loading_bootstraps <- data.frame(
      foreach(
        ...
        loading_bootstraps_tablebegin <- loading_bootstraps_begin[i] # CTC edited: 1/8/2026
        
        load.designation <- str_extract(
          results_astable[loading_bootstraps_begin[i],1],
          # pattern = "(?<=other_loads_)(.*?)(?=_rev)"
          pattern = "(?<=inc_storage_load_)(.*?)(?=_rev)" # CTC edited 1/8/2026
        )
        ...
        if(
          ...
          loading_bootstraps.tmp <- data.frame(
            str_split_fixed(
              str_squish( # CTC edited: 1/8/2026
              parse_character(
                results_astable[
              ...
              # ), " ", n = 2
              )), " ", n = 4 # CTC edited: 1/8/2026
            )
          ) %>%
            # rename(Bootstrap = 1, Load = 2) %>%
            rename(Bootstrap = 1, Season = 2, Year = 3, Load = 4) %>% # CTC edited: 1/8/2026
            mutate(
              across(.fns = ~as.numeric(as.character(.))),
              Nutrient = load.nutrient,
              Number = load.number
            ) %>%
            group_by(Bootstrap, Year, Nutrient, Number) %>% # CTC edited: 1/8/2026 Note, included to create annual estimates
            summarize(Load = sum(Load), .groups = "keep") %>% 
            ungroup()
...
mung_UserSpecs_loadingtargets <- function(loadingtargets) {
  UserSpecs_loadingtargets_munged <- loadingtargets %>%
    ...
    # group_by(Nutrient) %>%
    # mutate(Number = cumsum(ticker)) %>% # CTC edited 1/6/2026
    select(watershed_name, Percent_Reduction, Nutrient, Number) %>%
    unique()
...
get_loading_uncertainty_message <- function(
  ...
  CI.summary <- loadings %>%
    ...
    group_by(Nutrient, Number, Limit, watershed_name, Percent_Reduction) %>%
    summarize(
...
read_postprocessor_urban_bmp_vec <-function(results_astable, scenarionumber) {
  ...
  urban_start_idx <- which(results_astable[, 1] == urban_txt)[scenarionumber]
  
  if(length(urban_start_idx) == 0) {
    return(character(0))
  }
  
  # First, collect variable definitions from comment lines
  variable_definitions <- list()
  current_idx <- urban_start_idx + 1
  
  # Parse comment lines that define variables (e.g., "# $10 = Porous_Pavement_w_underdrain")
  while(current_idx <= nrow(results_astable) && 
        grepl("^#", results_astable[current_idx, 1])) {
    comment_line <- results_astable[current_idx, 1]
    if(grepl("\\$\\d+\\s*=", comment_line)) {
      # Extract variable name and definition
      var_match <- regmatches(comment_line, regexpr("\\$\\d+", comment_line))
      def_match <- regmatches(comment_line, regexpr("=\\s*(.+)$", comment_line))
      if(length(var_match) > 0 && length(def_match) > 0) {
        var_name <- var_match[1]
        var_def <- gsub("^=\\s*", "", def_match[1])
  ...
  # Find the header line (contains ":=" at the end)
  header_line <- ""
  while(current_idx <= nrow(results_astable)) {
    if(grepl(":=$", results_astable[current_idx, 1])) {
      header_line <- results_astable[current_idx, 1]
      break
    }
    current_idx <- current_idx + 1
  }
  ...
  # Replace variable references with actual names
  urban_bmp_vec <- urban_bmp_vec_tmp1
  for(i in seq_along(urban_bmp_vec)) {
    if(urban_bmp_vec[i] %in% names(variable_definitions)) {
      urban_bmp_vec[i] <- variable_definitions[[urban_bmp_vec[i]]]
    }
  }
...
parse_urban_results <- function(
  ...
  if(length(urban_rows_begin) > 0) {
    ...
    # Find the start of data rows (after the header line with ":=")
    data_start_idx <- urban_rows_begin + 1
    while(data_start_idx < urban_rows_end && 
          !grepl(":=$", results_astable[data_start_idx, 1])) {
      data_start_idx <- data_start_idx + 1
    }
    
    # Data starts on the line after the header
    data_start_idx <- data_start_idx + 1
    
    if(data_start_idx >= urban_rows_end) {
      return(NULL)
    }
    
    urban_rows <- results_astable[
      data_start_idx:(urban_rows_end - 1), 1
    ]
...
read_postprocessor_ripbuf_bmp_vec <- function(results_astable, scenarionumber) {
  ...
  ripbuf_txt <- "ripbuf_length [*,*]" # CTC: edited 12-30-25
  ripbuf_bmp_tmp <- results_astable[(which(results_astable[, 1] == ripbuf_txt) + 1), 1][scenarionumber]
...
parse_ripbuf_results <- function(
  ...
  if(length(ripbuf_rows_begin) > 0 & !is.na(ripbuf_rows_begin)) {
    ...
    ripbuf_rows <- results_astable[
      # (ripbuf_rows_begin + 1):(ripbuf_rows_end - 1), 1
      (ripbuf_rows_begin + 2):(ripbuf_rows_end - 1), 1 # CTC: edited 12-30-2025
    ]
    ...
    # names(ripbuf_results) <- c("COMID", riparianbmps, "Length")
    names(ripbuf_results) <- c("COMID", riparianbmps) # CTC: edited 12-30-2025
    
    # ripbuf_results <- ripbuf_results %>%
    #   pivot_wider(names_from = riparianbmps, values_from = "Length") # CTC: edited 12-30-2025
...
# Adding in functions to parse septic results from NEOS file
read_postprocessor_septic_bmp_vec <- function(results_astable, scenarionumber) {
  
  # septic_txt <- "septic_frac :="
  septic_txt <- "septic_frac [*,*]" # CTC: edited 12-30-2025
  septic_bmp_tmp <- results_astable[
    (which(results_astable[, 1] == septic_txt) + 1), 1
  ][scenarionumber]
  
  if(
    grepl(
      "ag_frac|urban_frac|point_dec|ripbuf_length|NEOS|Presolve", septic_bmp_tmp
    )
  ) {septic_bmp_tmp <- ""}
  
  septic_bmp <- strsplit(septic_bmp_tmp, "\\s+")
  septic_bmp_vec_tmp <- as.vector(unlist(septic_bmp))
  septic_bmp_vec_tmp1 <- septic_bmp_vec_tmp[
    !sapply(septic_bmp_vec_tmp, function(x) all(x == ""))
  ]
  septic_bmp_vec <- septic_bmp_vec_tmp1[-c(1, length(septic_bmp_vec_tmp1))]
  
  return(septic_bmp_vec)
}

parse_septic_results <- function(results_astable, scenarionumber, endrows, septicbmps) {
  
  # septic_txt <- "septic_frac :="
  septic_txt <- "septic_frac [*,*]" # CTC: edited 12-30-2025
  
  septic_rows_begin <- which(results_astable[, 1] == septic_txt)[scenarionumber]
  
  if(length(septic_rows_begin) > 0) {
    
    septic_rows_end <- min(endrows[which(endrows > septic_rows_begin)])
    
    septic_rows <- results_astable[
      (septic_rows_begin + 2):(septic_rows_end - 1), 1
    ]
    
    septic_tmp <- strsplit(septic_rows, "\\s+")
    
    septic_results <- do.call(rbind.data.frame, septic_tmp)
    
    names(septic_results) <- c("COMID", septicbmps)
    
    septic_results$COMID_adj <- gsub("'", "", septic_results$COMID)
    septic_results$COMID_int <- strtoi(septic_results$COMID_adj)
    septic_results <- septic_results %>%
      select(COMID = COMID_int, all_of(septicbmps))
    
  } else {septic_results <- NULL}
  
  return(septic_results)
}

merge_septic_results <- function(septicresults, areadf, septicbmps, km2_to_ac) {
  
  # Merge the COMID to HUC12 crosswalk to the septic BMP results
  septic_results_HUCmerge <- merge(
    septicresults, areadf, by = "COMID", all.x = TRUE
  )
  
  # Calculate BMP implementation (parcels) by HUC12
  for (i in 1:length(septicbmps)) {
    septic_results_HUCmerge[septicbmps[i]] <- sapply(
      sapply(septic_results_HUCmerge[septicbmps[i]], as.character), 
      as.numeric
    )
    colname <- paste(septicbmps[i], "parcels", sep = "_")
    septic_results_HUCmerge[colname] <- septic_results_HUCmerge[septicbmps[i]]
  }
  
  foreach(i = 1:length(septicbmps)) %do% {
    attr(septic_results_HUCmerge[, septicbmps[i]], 'dimnames') <- NULL
  }
  
  return(septic_results_HUCmerge)
}

present_septic_results_HUC <- function(septicresults, septicbmps) {
  septic_results_HUC <- data.frame(septicresults) %>%
    rename_at(
      vars(one_of(septicbmps)), list( ~ paste0(., "_ParcelsImplement"))
    ) %>%
    select(
      COMID,
      HUC_12,
  ...
  return(septic_results_HUC)
}

calculate_septic_total <- function(septicresults, septicbmps) {
  
  septic_total <- data.frame(
    lapply(
      septicresults[paste0(septicbmps, "_parcels")], sum
    )
  )
  
  septic_total[paste0(septicbmps, "_parcels")] <- format(
    septic_total[paste0(septicbmps, "_parcels")],
    big.mark = ',',
    scientific = FALSE,
    # digits = 0, 
    digits = NULL, # CTC: edited 12-30-2025
    nsmall = 0
  )
  
  names(septic_total) <- gsub("_", " ", paste0(septicbmps, "_parcels"))
  names(septic_total) <- gsub("parcels", "(parcels)", names(septic_total))
...
#@ SE: Revised to remove error from map_dfr function 7-17-25
make_areadf <- function(sparrowinputs, km2_to_ac, streamcatinputs) {
  ...
  if (is.data.frame(streamcatinputs)) {
    # Single file case - already has StateAbbrev column
    streamcat_combined <- streamcatinputs
  } else if (is.list(streamcatinputs)) {
    # Multiple files case - combine with state abbreviations
    streamcat_combined <- imap_dfr(
      streamcatinputs, 
      ~ {
        if (is.data.frame(.x)) {
          return(.x %>% mutate(StateAbbrev = str_sub(.y, 1, 2)))
        } else {
          return(NULL)
        }
      }
...
render_results_display <- function(loadingtargets_pulled) {
  mainPanel(
    ...
    em(paste("To save this report, open the RBEROST postprocessor in a web",
             "browser and use the print to pdf functionality.")),
    wellPanel(
      h4("User Notes:"),
        ...
        fluidRow(
          ...
          p(
            paste(
              ...
              "Sometimes this may be a result of user error during",
              "specification of user inputs (e.g. in the case of nonsensical",
              "limits on BMPs) or during NEOS submission (e.g. uploading",
              "incorrect files). Other times, this is a result of loading",
              "targets that are too restrictive, or a result of not",
              "including enough BMPs for the model to use. It is also",
              "possible to receive an error for reasons other than those",
              "listed above. Please check your user inputs and refer to the",
              "Model Sensitivity section of the documentation for more",
              "information. If problems persist a user might try running",
              "RBEROST again with less restrictive loading targets and/or",
              "including more BMPs in the optimization."
            ),
      ...
      condition = "output.solvedstatus == 2", {
        fluidRow(
          ...
          p(
            paste(
              "RBEROST may fail to optimize models for a variety of reasons.",
              "Often, this is a result of loading targets that are too",
              "restrictive, or a result of not including enough BMPs for the",
              "model to use. It may be informative to run RBEROST again with",
              "less restrictive loading targets and/or including more BMPs",
              "in the optimization. Please check your user inputs and refer",
              "to the Model Sensitivity section of the documentation for",
              "more information."
            )
          )
        )
        ...
        fluidRow(
          ...
          conditionalPanel(
            ...
            wellPanel(
              ...
              p(
                paste("Below is the probability density distribution of cost",
          ...
          conditionalPanel(
            ...
            wellPanel(
              ...
              p(
                paste("Below are the probability density distributions of",
                      ...
                      "and probability densities to the right of the grey",
                      "line are load estimates that exceed the loading target.")
              ),
              plotOutput(
          ...
          conditionalPanel(
            ...
            fluidRow(
              p(
                paste("The model chose to implement WWTP retrofits at the",
                      "following locations:")
              ),
              tableOutput("wwtp")
            )
          ...
          conditionalPanel(
            ...
            fluidRow(
              p(
                paste("The model chose to implement the total area of the",
                      "following agricultural BMPs:")
              ),
              tableOutput("ag"),
              downloadLink(
          ...
          conditionalPanel(
            ...
            fluidRow(
              p(
                paste("The model chose to implement the total length of the",
                      "following riparian buffer BMPs.")
              ),
              tableOutput("ripbuf"),
              downloadLink(
          ...
          tags$hr(),
          conditionalPanel(
            condition = "output.septicdownloadavail",
            fluidRow(
              p(
                paste("The model chose to implement the total number of",
                      "parcels for the following septic BMPs.")
              ),
              tableOutput("septic"),
              downloadLink(
                "downloadsepticdat", "Download Septic BMPs by COMID"
              )
            )
          ),
          conditionalPanel(
            condition = "!output.septicdownloadavail",
            p("The model did not implement any septic BMPs.")
          )
        )
      }
    )
