...
region1 <- "NHDPlusMA/NHDPlusV21_MA_02_NHDSnapshot_04.7z"
region2 <- "NHDPlusNE/NHDPlusV21_NE_01_NHDSnapshot_04.7z"
...
# ND: Switched to 2016 to be consistent with documentation for dynamic LIS RBEROST
...
riparianloadings_seasonal <- riparianloadings %>%
  left_join(sparrow_cons_out_tn %>% select(COMID, season, year, in_total, in_fert_ag, in_fert_ag_se), by="COMID") %>% 
  left_join(sparrow_cons_out_tp %>% select(COMID, season, year, ip, ip_fert, ip_forest, ip_fert_se, ip_forest_se), by=c("COMID", "season", "year")) %>%
  mutate(
