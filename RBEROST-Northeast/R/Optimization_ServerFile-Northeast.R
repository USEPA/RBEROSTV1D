...
source("./RBEROST-Northeast/R/Optimization_ServerFunctions_Postprocessor-Northeast.R")
...
server <- function(input, output, session) {
  ...
  # Updated to handle single streamcat file
  NEOS_renamed <- eventReactive(
    eventExpr = {input$ViewReport},
    valueExpr = {
      # Handle both single dataframe and list cases for streamcat files
      streamcat_check <- if (is.data.frame(streamcat_files_all_reactive())) {
        nrow(streamcat_files_all_reactive()) > 0
      } else if (is.list(streamcat_files_all_reactive())) {
        all(unlist(lapply(streamcat_files_all_reactive(), nrow)) > 0)
      } else {
        FALSE
      }
...
septic_bmp_vec <- reactive(
  {
    req(NEOS_renamed, solved()[scen.vis()] == 1)
    read_postprocessor_septic_bmp_vec(NEOS_renamed(), scen.vis()) 
  }
)

septic_results <- reactive(
  {
    req(
      NEOS_renamed,
      NEOS_end_rows,
      length(septic_bmp_vec()) > 0,
      solved()[scen.vis()] == 1
    )
    parse_septic_results(
      NEOS_renamed(), scen.vis(), NEOS_end_rows(), septic_bmp_vec()
    )
  }
)
  ...
  area <- eventReactive(
    ...
    {
      # Handle both single dataframe and list cases for streamcat files
      streamcat_ncol_check <- if (is.data.frame(streamcat_files_all_reactive())) {
        TRUE  # Single dataframe, no need to check column consistency
      } else if (is.list(streamcat_files_all_reactive())) {
        all(
          diff(
            unlist(
...
septic_results_HUCmerge <- reactive(
  {
    req(!is.null(septic_results), area, septic_bmp_vec, solved()[scen.vis()] == 1)
    merge_septic_results(septic_results(), area(), septic_bmp_vec(), km2_to_ac)
  }
)

septic_results_HUC <- reactive(
  {
    req(
      septic_results_HUCmerge, 
      length(septic_bmp_vec()) > 0,
      solved()[scen.vis()] == 1
    )
    present_septic_results_HUC(septic_results_HUCmerge(), septic_bmp_vec())
  }
)

septic_total <- reactive(
  {
    req(septic_results_HUCmerge, septic_bmp_vec, solved()[scen.vis()] == 1)
    calculate_septic_total(septic_results_HUCmerge(), septic_bmp_vec()) 
  }
)
      ...
      isolate(
        validate(
          ...
          if(length(input$streamcat_crop_files_all) > 0) {
            # Check if files contain valid State column with state abbreviations
            streamcat_data <- streamcat_files_all_reactive()
            
            # Handle both single file and multiple files cases
            files_to_check <- if(is.data.frame(streamcat_data)) {
              list(streamcat_data)
            } else {
              streamcat_data
            }
            
            # Check each file has State column with valid state abbreviations
            valid_states_check <- sapply(files_to_check, function(df) {
              has_state_col <- "State" %in% names(df)
              if (has_state_col) {
                unique_states <- unique(df$State)
                all(unique_states %in% state.abb)
              } else {
                FALSE
              }
            })
            ...
            need(
              all(valid_states_check), 
              paste0("StreamCat files must contain a 'State' column with valid ",
                     "two-letter state abbreviations. Please check your file ",
                     "content and ensure the State column contains only valid ",
                     "state abbreviations (e.g., VT, NH, CT, etc.).")
            )
          }
        )
      )
...
output$septic <- renderTable(
  expr = {
    req(septic_total, length(septic_bmp_vec()) > 0)
    return(value = septic_total())
  }
)

output$septicdownloadavail <- reactive(
  {req(septic_results_HUC); nrow(septic_results_HUC()) > 0}
)

outputOptions(output, "septicdownloadavail", suspendWhenHidden = FALSE)

output$downloadsepticdat <- downloadHandler(
  filename = reactive(
    {paste0("SepticBMP_by_ComID_Scenario", scen.vis(), ".csv")}
  ),
  content = function(file) {write.csv(septic_results_HUC(), file)}
)
  ...
  output$report <- renderUI(
    {
      ...
      isolate(
        validate(
          ...
          need(
            all(input$streamcat_crop_files_all$size < 100000000),
            paste(paste0(input$streamcat_crop_files_all$name[which(!(input$streamcat_crop_files_all$size < 100000000))], collapse = ", "), "StreamCat file size(s) detected as unusually large. Please double check inputs.")
          )
        )
      )
      
      ## Check formatings ####
      #@ SE: Create new StreamCat file validations for updated input file -------------------------------------------
      isolate(
        validate(
          need(
            scen.vis() <= num.scen(),
          ...
          need(
            all(
              c("watershed_name", "ComID", "Percent_Reduction", "TN_or_TP") %in%
          ...
          # # Updated validation for StreamCat files
          # need(
          #   if(is.list(streamcat_files_all_reactive())) {
          #     all(
          #       diff(
          #         unlist(
          #           lapply(streamcat_files_all_reactive(), ncol),
          ...
          #     )
          #   } else {
          #     TRUE  # Single file case
          #   },
          #   "Number of columns in state Streamcat files differ. Please check inputs and formatting."
          # ),
          # # Updated duplicate file check - only for multiple files
          # need(
          #   if(is.list(streamcat_files_all_reactive()) && length(streamcat_files_all_reactive()) > 1) {
          #     # Extract potential state abbreviations (first 2 characters)
          #     file_prefixes <- str_sub(names(streamcat_files_all_reactive()), 1, 2)
          #     # Check if they're valid state abbreviations and not duplicated
          #     valid_states <- file_prefixes %in% state.abb
          #     if(any(valid_states)) {
          #       length(which(duplicated(file_prefixes[valid_states]))) == 0
          #     } else {
          #       # If no valid state abbreviations found, provide a more helpful error
          #       FALSE
          #     }
          #   } else {
          #     TRUE  # Single file or no files
          #   },
          #   paste0(
          #     "StreamCat file naming issue detected. Files should be named starting with 2-letter state abbreviations (e.g., 'VT_StreamCat.csv', 'NH_StreamCat.csv'). ",
          #     "Current file names: ", 
          #     paste0(names(streamcat_files_all_reactive()), collapse = ", "),
          #     ". Please rename your files to start with valid state abbreviations."
          #   )
          # ),
