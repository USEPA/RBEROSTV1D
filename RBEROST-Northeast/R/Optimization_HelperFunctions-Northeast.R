...
### Speed is improved by pre-filtering the sparrow input file to only comids within the HUC2 of the target comid

get_upstream_ComIDs <- function(sparrow_in, target.comid) {

  HUC2_of_target <- substr(
    str_pad(
      if("HUC_12" %in% names(sparrow_in)) {
        sparrow_in$HUC_12[which(sparrow_in$comid == target.comid)]
        } else if("huc12" %in% names(sparrow_in)) {
          sparrow_in$huc12[which(sparrow_in$comid == target.comid)]
        },
      width=12,
      pad="0"
    ), 
  ...
  reaches <- sparrow_in %>% 
    mutate(
      Huc2 = if("HUC_12" %in% names(sparrow_in)) {
        substr(as.character(str_pad(HUC_12, width=12, pad="0")), 1,2)
      } else if("huc12" %in% names(sparrow_in)) {
          substr(as.character(str_pad(huc12, width=12, pad="0")), 1,2)
        }
    ) %>%
    filter(Huc2 == HUC2_of_target) 

  river.graph <- as_tbl_graph(
    reaches %>% select(to = ctonode, from = cfromnode, comid)
    )
